...
namespace Microsoft.JSInterop.Test.DotNetDispatcherTests
{
    public class DotNetDispatcherInstanceMethodTest : DotNetDispatcherBaseTest
    {
        ...
        public Task CanInvokeInstanceMethodWithParams() => WithJSRuntime(jsRuntime =>
        {
            ...
            string argsJson = "[\"myvalue\",\"__dotNetObject:2\"]";
        ...
        [Fact]
        public Task CanInvokePublicInstanceVoidMethodOnGenericClass() => WithJSRuntime(jsRuntime =>
        {
            // Arrange: Track some instance
            var targetInstance = new PublicGenericClass<int>();
            jsRuntime.Invoke<object>("unimportant", new DotNetObjectRef(targetInstance));

            // Act
            var resultJson = DotNetDispatcher.Invoke(null, TestModelMethodNames.PublicGenericClass_PublicInstanceVoidMethod, 1, null);
        ...
        [Fact]
        public Task CanInvokePublicInstanceNonVoidMethodOnGenericClass() => WithJSRuntime(jsRuntime =>
        {
            // Arrange: Track some instance
            var targetInstance = new PublicGenericClass<bool>();
            jsRuntime.Invoke<object>("unimportant", new DotNetObjectRef(targetInstance));

            // Act
            var resultJson = DotNetDispatcher.Invoke(null, TestModelMethodNames.PublicGenericClass_PublicInstanceNonVoidMethod, 1, null);

            // Assert
            Assert.Equal("false", resultJson);
            Assert.True(targetInstance.InstanceMethodWasInvoked);
        });

        [Fact]
        public Task CanInvokePublicInstanceNonVoidMethodOnGenericClassWithParams() => WithJSRuntime(jsRuntime =>
        {
            // Arrange: Track some instance
            var targetInstance = new PublicGenericClass<string>();
            jsRuntime.Invoke<object>("unimportant", new DotNetObjectRef(targetInstance));
            string argsJson = "[\"SomeValue\"]";

            // Act
            var resultJson = DotNetDispatcher.Invoke(null, TestModelMethodNames.PublicGenericClass_PublicInstanceNonVoidMethodWithParams, 1, argsJson);

            // Assert
            Assert.Equal("\"SomeValue\"", resultJson);
            Assert.True(targetInstance.InstanceMethodWasInvoked);
        });
